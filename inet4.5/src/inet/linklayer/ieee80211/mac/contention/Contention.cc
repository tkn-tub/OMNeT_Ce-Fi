...
// SWSH TODO: NDF Overload the startContention function with an additional parameter for starting
// SWSH ADDED: Function for the extended contention
void Contention::startExtendedContention(int cw, simtime_t ifs, simtime_t eifs, simtime_t slotTime, ICallback *callback, int cw_offset) // alternativ omega mitgeben, um offset lokal zu berechnen
{
//    Enter_Method("startExtendedContention");
//    cancelTransmissionRequest(); //-> Muss an einer anderen Stelle gecancelled werden (wirft bug auf)
    startTime = simTime();
    ASSERT(ifs >= 0 && eifs >= 0 && slotTime >= 0 && cw >= 0);
    Enter_Method("startContention");
    cancelEvent(channelGrantedEvent);
    ...
    // SWSH Changed: Add an Offset to the CW to increase
    backoffSlots = intrand(cw + 1) + cw_offset;
    // chaEnd
    emit(backoffPeriodGeneratedSignal, backoffSlots);
    EV_DETAIL << "Starting FT contention: cw = " << cw << ", slots = " << backoffSlots << ", slotTime = " << slotTime << ", ifs = " << ifs << ", eifs = " << eifs << endl;
    handleWithFSM(START);
}
// addEnd

// SWSH ADDED: Stop contention completely for a "regular" contention to take place
void Contention::cancelContention(){
    EV_DEBUG << "Cancel Transmission in Contention" << endl;
    fsm.setState(IDLE, "IDLE");
    EV_DEBUG << "\nOwning context of startTxEvent before: " << startTxEvent->getOwningContext() <<"\nOwner of startTxEvent before: " << startTxEvent->getOwner() << endl;
    startTxEvent->setOwningContext(this);
    EV_DEBUG << "\nOwning context of startTxEvent after: " << startTxEvent->getOwningContext() <<"\nOwner of startTxEvent after: " << startTxEvent->getOwner() << endl;
    ...
    FSMA_Switch(fsm) {
        ...
        FSMA_State(DEFER) {
            ...
            FSMA_Event_Transition(Use-EIFS,
                    ...
                    // SWSH ADDED: 
                    FSMA_Ignore_Event(event==MEDIUM_STATE_CHANGED && !mediumFree);
                    // addEnd
            FSMA_Fail_On_Unhandled_Event();
        }
