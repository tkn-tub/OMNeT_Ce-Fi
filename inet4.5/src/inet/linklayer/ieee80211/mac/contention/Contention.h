...
class INET_API Contention : public cSimpleModule, public IContention
{
  ...
  public:
    ...
    // SWSH ADDED: Alternative contention function for Forced Traffic Mechanism
    virtual void startExtendedContention(int cw, simtime_t ifs, simtime_t eifs, simtime_t slotTime, ICallback *callback, int omega) override;
    // SWSH ADDED: cancel current contention to start a new one
    virtual void cancelContention() override;
    // SWSN ADDED: Added to get the current backoffSlots
    virtual int getBackoffSlots() override { return backoffSlots; }
    // addEnd
