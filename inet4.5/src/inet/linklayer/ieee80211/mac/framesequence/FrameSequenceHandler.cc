...
void FrameSequenceHandler::handleStartRxTimeout()
{
    ...
    switch (lastStep->getType()) {
        ...
        case IFrameSequenceStep::Type::TRANSMIT:{
            // SWSH CHANGED: To cope with normally unintended behavior
            EV_DEBUG << "An corrupted frame was received but it is being ignored because another transmission is ongoing and data is on the FH." << endl;
            break;
...
void FrameSequenceHandler::processResponse(Packet *frame)
{
    ...
    switch (lastStep->getType()) {
        ...
        case IFrameSequenceStep::Type::TRANSMIT:{
            // SWSH CHANGED
            EV_DEBUG << "A packet got in but it got ignored because another transmission is ongoing" << endl;
            delete frame;
            break;
