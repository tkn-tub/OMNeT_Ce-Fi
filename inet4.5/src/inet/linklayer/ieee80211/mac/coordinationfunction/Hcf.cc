...
void Hcf::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        // SWSH ADDED: Necessary timer to cope with FH-delay
...
void Hcf::enqueueFtNdfFrame() {
//    Enter_Method("enqueueFtNdfFrame");
//    // Create Frame
//    auto ftNdfFrame = makeShared<Ieee80211NullDataHeader>();
//    // Brauche hier die Adresse des AP
//    inet::MacAddress receiverAddress = inet::MacAddress(apAddress.c_str());
//    auto transmitterAddress = mac->getAddress();
//    ftNdfFrame->setTransmitterAddress(transmitterAddress);
...
void Hcf::txTransmitFrame(Packet *packet, const Ptr<const Ieee80211MacHeader>& header, simtime_t ifs, const IIeee80211Mode *mode, ITx::ICallback *txCallback){
    //auto channelOwner = edca->getChannelOwner();
    //auto txop = channelOwner->getTxopProcedure();


// Abfrage ob response einfügen:
    // responseMode = rateSelection->computeResponseAckFrameMode(receivedPacket, dataOrMgmtHeader);
    // alternativ: Mode mit übergeben -> Dann wird die Funktion nur sehr lang
    ...
    if (!endTransmissionTimer->isScheduled()){
        ...
        scheduleAfter(ifs+transmissionDuration, endTransmissionTimer); // TEST
...
void Hcf::handleMessage(cMessage *msg)
{
    ...
    // SWSH ADDED: To handle the transmission timer
    else if (msg == endTransmissionTimer){
            EV_DEBUG << "The transmission should have ended" << endl;
...
void Hcf::transmitFrame(Packet *packet, simtime_t ifs)
{
    ...
    if (channelOwner) {
        ...
        // SWSH CHANGED: To be able to work with the FH-delay
        txTransmitFrame(packet, packet->peekAtFront<Ieee80211MacHeader>(), ifs, mode, this); //tx->transmitFrame(packet, packet->peekAtFront<Ieee80211MacHeader>(), ifs, this);
        // chaEnd
...
void Hcf::transmitControlResponseFrame(Packet *responsePacket, const Ptr<const Ieee80211MacHeader>& responseHeader, Packet *receivedPacket, const Ptr<const Ieee80211MacHeader>& receivedHeader)
{
    ...
    txTransmitFrame(responsePacket, responseHeader, modeSet->getSifsTime(), responseMode, this);// tx->transmitFrame(responsePacket, responseHeader, modeSet->getSifsTime(), this);
    // chaEnd
