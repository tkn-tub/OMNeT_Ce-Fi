...
class INET_API Dcaf : public IChannelAccess, public IContention::ICallback, public IRecoveryProcedure::ICwCalculator, public ModeSetListener
{
  ...
  public:
    ...
    // SWSH ADDED:
    virtual void incrementCwOffset() override;
    // SWSH ADDED:
    virtual void resetCwOffset() override;
    // SWSH ADDED:
    virtual void nullCwOffset() override;
    //SWSH ADDED:
    virtual int getCwOffset() override {return cwOffset; }
    // SWSH ADDED:
    virtual int getBackoffSlots();

    // SWSH ADDED:
    virtual void cancelContention();
};
