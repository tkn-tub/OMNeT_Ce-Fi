...
class INET_API Rx : public cSimpleModule, public IRx
{
  ...
  protected:
    ...
    // SWSH ADDED: necessary variables to work with FH-delay
    bool mediumFreeAtMac;
    simtime_t fhDelay;
    // addEnd
  ...
  protected:
    ...
    // SWSH ADDED: startMediumFree-timer function
    virtual void startMediumFreeTimer(bool mediumFree);
    // addEnd
  ...
  public:
    ...
    // SWSH ADDED: Function to get and notify about the medium status
    virtual bool getMediumStatusAtMac() override; // To check the medium status as seen by the MAC in a FH-szenario
    virtual void notifyContentionOfMediumChange(bool mediumFree) override;
    // addEnd
};
