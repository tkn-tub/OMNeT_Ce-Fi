...
{
    //SWSH ADDED for statistics about collisions
    corruptedFrameSignal = registerSignal("corruptedFrameSignal");
    //SWSH ADDEND
...
void Ieee80211Mac::receiveSignal(cComponent *source, simsignal_t signalID, intval_t value, cObject *details)
{
    ...
    if (signalID == IRadio::receptionStateChangedSignal) {
        ...
        if (transmissionFinished) {
            //tx->radioTransmissionFinished(); // SWSH RMVD: Not compatible with the correct representation of FH
            EV_DEBUG << "changing radio to receiver mode\n";
            //configureRadioMode(IRadio::RADIO_MODE_RECEIVER); // FIXME this is in a very wrong place!!! should be done explicitly from coordination function!// SWSH RVMD: Not compatible with the correct representation of FH
        }
...
{
    // SWSH ADDED: Need to access configure Radio Mode from outside
    Enter_Method("configureRadioMode");
    auto currentRadioMode = radio->getRadioMode();
    EV_DEBUG << "Current Radio Mode: " << currentRadioMode << endl;
    // addEnd

    //if (radio->getRadioMode() != radioMode) { // SWSH Removed: Removed because the condition does not hold true with FH
        ...
        sendDown(request);
    //} // SWSH RMVD: Does not hold true with FH
}
